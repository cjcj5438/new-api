...
func (p *RetryParam) ExcludedChannelIDs() []int {
	if p == nil || p.Ctx == nil {
		return nil
	}
	usedChannels := p.Ctx.GetStringSlice("use_channel")
	if len(usedChannels) == 0 {
		return nil
	}
	ids := make([]int, 0, len(usedChannels))
	seen := make(map[int]struct{}, len(usedChannels))
